      ...
      <VisualFrame label="Weekend flow">
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            ...
            <div key={number} className="border border-border bg-surface-02 p-4">
            ...
            {["P1", "P2", "P3", "P4", "P5", "P6"].map((position, index) => (
              ...
              >
                ...
                <span
                  className={
                    index === 0
                      ? "ml-auto h-2 w-10 bg-race-red"
                      : "ml-auto h-2 w-10 bg-text-muted/40"
                  }
                />
              </div>
            ))}
    ...
    return (
      <VisualFrame label="Overtaking opportunity">
        <div className="relative h-44 overflow-hidden border border-border bg-surface-02">
          ...
          <div className="absolute left-[18%] top-[44%] h-4 w-16 bg-white" />
          <div className="absolute left-[44%] top-[56%] h-4 w-16 bg-race-red" />
          <div className="absolute left-[57%] top-[58%] h-px w-[20%] bg-race-red" />
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={name} className="text-center">
              <div
                className={`mx-auto flex aspect-square max-w-28 items-center justify-center rounded-full border-[10px] ${borderClass} bg-surface-02`}
              >
      ...
      <div className="relative grid gap-3 sm:grid-cols-2">
        {["The start", "Tyres & pits", "Overtaking areas", "Final laps"].map(
          (label, index) => (
            <div
              key={label}
              className="flex min-h-16 items-center gap-4 border border-border bg-surface-02 px-4"
            >
...
function VisualFrame({
  ...
  return (
    <div className="rounded-[6px] border border-border bg-surface-01 p-5 sm:p-6">
...
function FlagTile({ label, className }: { label: string; className: string }) {
  return (
    <div className={`flex min-h-24 items-end p-3 ${className}`}>
